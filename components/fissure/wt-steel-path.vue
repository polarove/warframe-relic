<template>
  <wt-context-menu-container>
    <wt-fissure
      ...
      @finish-steelpath="(expired: Fissure) => clearFissure(expired)"
    >
      <div class="flex-center">
        ...
        <span v-if="!steelpath.state.updating"> {{ steelpath.state.tip }}</span>
      </div>
    </wt-fissure>
  </wt-context-menu-container>
  <wt-spirit />
...
const checkDataState = (fissures: Fissure[]) => {
  steelpath.state = steelpath.fissure.every((fissure) =>
    isValidFissure(fissure)
  )
    ? DATA_CLEAN
    : DATA_OUTDATED

  if (steelpath.state) return Promise.resolve(fissures)
...
const currentFissures = () => steelpath.fissure

const fillFissure = (fissures: Fissure[]) => {
  if (ListUtil.isEmpty(fissures)) steelpath.empty = true
  updateFissure(fissures).finally(() => (steelpath.loading = false))
}
...
const updateState = (name: string, tip: string, state: FissureDataState) => {
  ...
  console.log(log)
  steelpath.state = {}
  steelpath.state = parseState(state, log)
  console.log(parseLog(name, JSON.stringify(steelpath.state)))
  logDivider()
  return log
}
